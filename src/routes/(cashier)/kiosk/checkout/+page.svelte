<script lang="ts">
  ...
  function updateQuantity(item: CartItem, qty: number) {
    cart.update((items: CartItem[]) =>
      items
        .map((c) => (c.id === item.id ? { ...c, qty: qty > 0 ? qty : 0 } : c))
        .filter((c) => c.qty > 0)
    );
  }
</script>
...
<div class="flex min-h-screen w-full">
  ...
  <aside class="w-64 bg-gradient-to-b from-purple-700 via-pink-600 to-indigo-600 text-white flex flex-col shadow-lg fixed h-screen">
    ...
    <nav class="flex-1 px-4 space-y-4">
      ...
      <a href="/kiosk/checkout" class="w-full block px-4 py-3 rounded-xl bg-yellow-100 text-blue-600 border-l-4 border-yellow-400">💳 Checkout</a>
      <a href="/kiosk/confirm" class="w-full block px-4 py-3 rounded-xl bg-white text-gray-900 hover:shadow-lg">✅ Confirm</a>
            ...
            <li class="flex justify-between items-center border-b pb-2">
              ...
              <div class="flex items-center gap-2">
                <input
                  ...
                  min="1"
                  bind:value={item.qty}
                  on:change={(e) => updateQuantity(item, +(e.target as HTMLInputElement).value)}
